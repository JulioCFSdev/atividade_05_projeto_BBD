...
def hud_score(
    display, size, pos_money_icon_var, score_value, pos_score_value_var
):
    ...
    display.blit(
        text2, (size - (size / pos_score_value_var + 70), -15)
    )  # 1.15
...
def hud_lives(
    display, size, pos_lives_icon_var, life_value, pos_lives_value_var
):
    text = conf.font2.render("💕", True, conf.white)
...
def screen_lines(display, color, line_size):
    ...
    # horizontal lines
    pygame.draw.line(
        display, color, [0, 70], [conf.screen_width, 70], line_size
    )

    # vertical lines
    pygame.draw.line(
        display, color, [0, 0], [0, conf.screen_height], line_size
    )
    pygame.draw.line(
        display,
        color,
